...
begin
	...
	-- POSIBLE SOLUCIÓN
		load_pc <= '0' when ((stall_MIPS = '1') OR (stall_ID = '1')) else
	 			   '1';
	...
	PC_in <= 	x"00000008" 		when (Exception_accepted = '1') and (Data_abort = '1') else -- If a data abort arrives we jump to address 0x00000008.
				x"0000000C" 		when (Exception_accepted = '1') and (UNDEF = '1') else -- If an UNDEF arrives, we jump to address 0x0000000C. 	AÑADIDO(DESCOMENTADO)
				x"00000004" 		when (Exception_accepted = '1') and (IRQ = '1') else -- If an IRQ arrives, jump to address 0x00000004 		AÑADIDO(DESCOMENTADO)
				Exception_LR_output when RTE_ID = '1' else 	--@ retorno. If it is an RTE we revert to the @ we had stored in the Exception_LR 	AÑADIDO(DESCOMENTADO)			
				busA				when RET_ID = '1' else 	--The address stored in the Rs register outgoing from port A of the BR is chosen. 	AÑADIDO(DESCOMENTADO, poner busA para PC)
	...
	-- POSIBLE SOLUCIÓN
	load_ID <= '0' when ((stall_MIPS = '1') OR (stall_ID = '1')) else
	 		   	'1';
	...
	-- POSIBLE SOLUCIÓN
	load_EX <= '1' when (stall_MIPS = '0') else
				 '0';
	...
	-- POSIBLE SOLUCIÓN
	load_MEM <= '1' when (stall_MIPS = '0') else
				 '0';
	...
	-- POSIBLE SOLUCIÓN
	load_WB <= '1' when (stall_MIPS = '0') else
				 '0';
	...
	ctrl_Mux4a1_escritura_BR <= jal_WB & MemtoReg_WB	; --AÑADIDO jal_WB por '0'
...
-------------------------------------------------------------------------------------------------- 
	...
	--POSIBLE SOLUCIÓN
	inc_I <= '1' WHEN ((valid_I_WB = '1') AND (stall_MIPS = '0')) else '0'; 
	--inc_data_stalls <= '0'; --Complete. Stall cycles due to data hazards
	--POSIBLE SOLUCIÓN
	inc_data_stalls <= '1' WHEN (stall_ID = '1') AND (stall_MIPS = '0') else '0';
	--inc_control_stalls <= '0'; --Complete.Stall cycles due to control hazards
	--POSIBLE SOLUCIÓN
	inc_control_stalls <= '1' WHEN ((Kill_IF = '1') AND (stall_MIPS = '0')) else '0';
	...
	--POSIBLE SOLUCIÓN
	inc_Mem_stalls <= '1' WHEN ((valid_I_MEM = '1') AND (IO_MEM_ready = '0')) else '0';
	--inc_EX_stalls <= '0'; --Complete. Structural Stall cycles generated by the multi-cycle MAC operations
	--POSIBLE SOLUCIÓN
	inc_EX_stalls <= '1' WHEN ((ALU_ready = '0') AND (valid_I_EX =  '1') AND (IO_MEM_ready = '1')) else '0';
