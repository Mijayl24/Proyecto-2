...
-- Register use: identifies if the current instruction reads Rs or Rt
	rs_read <= '1' when ((IR_op_code = ARIT_opcode) or (IR_op_code = LW_opcode) or (IR_op_code = SW_opcode) or (IR_op_code = BEQ_opcode) or (IR_op_code = RET_opcode)) else '0'; --No se usa (IR_op_code = FI_opcode)
...
-- Data hazards COMPLETE:
	...
	-- BEQ_rs	<= 	'0';
	-- POSIBLE SOLUCIÓN
	BEQ_rs <= '1' when ((IR_op_code = BEQ_opcode) AND (valid_I_ID = '1') AND (dep_rs_EX = '1' OR dep_rs_Mem = '1')) else '0';

	-- BEQ_rt	<= 	'0';
	--POSIBLE SOLUCIÓN
	BEQ_rt   <= '1' when ((IR_op_code = BEQ_opcode) AND (valid_I_ID = '1') AND (dep_rt_EX = '1' OR dep_rt_Mem = '1')) else '0';
	-- 3) RET: Similar to beq hazard, but taking into account that RET only uses Rs
	
	-- RET_rs	<= 	'0';
	--POSIBLE SOLUCIÓN
	RET_rs <= '1' when ((IR_op_code = RET_opcode) AND  (valid_I_ID = '1') AND (dep_rs_EX = '1' OR dep_rs_Mem = '1')) else '0';
	...
	-- JAL_uso_rs	<= 	'0';
	--POSIBLE SOLUCIÓN
	JAL_uso_rs <= '1' when ((valid_I_ID = '1') AND (rs_read = '1') AND ((JAL_EX  = '1' AND valid_I_EX  = '1' AND Reg_Rs_ID = RW_EX)  OR (JAL_MEM = '1' AND valid_I_MEM = '1' AND Reg_Rs_ID = RW_Mem))) else '0';
	
	-- JAL_uso_rt <= 	'0';
	--POSIBLE SOLUCIÓN
	JAL_uso_rt <= '1' when ((valid_I_ID = '1') AND (rt_read = '1') AND ((JAL_EX  = '1' AND valid_I_EX  = '1' AND Reg_Rt_ID = RW_EX)  OR (JAL_MEM = '1' AND valid_I_MEM = '1' AND Reg_Rt_ID = RW_Mem))) else '0';
...
-- stall_MIPS_internal is defined to be readable in the code (in vhdl the outputs of an entity cannot be read inside the entity. 
	stall_MIPS_internal <= (not(IO_MEM_ready)and valid_I_MEM) or (not(ALU_ready) and valid_I_EX); -- We also have to stop the processor when the ALU is not ready, for example in a division operation that takes several cycles. In this case we also have to stop the whole processor for the same reason as in the memory case: if we don't stop it, we may lose anticipated data.
